...
local function AddItems(stash, Items)
    ...
    -- if GetResourceState("qs-inventory"):match("start") then
    --     local itemNames = exports['qs-inventory']:GetItemList()
    --     local owner = stash:match("BennyShop_") and stash:gsub("BennyShop_", "") or false
    --     local inventory = exports['qs-inventory']:GetInventory(player)
    --     local label = stash:match("BennyShop_") and "Benny Shop" or "Crate"
    --     if not inventory then
    --         exports['qs-inventory']:RegisterStash(stash, label, 50, 100000)
    --     end
    --     for k, v in pairs(Items) do
    --         local item = itemNames[k:lower()]
